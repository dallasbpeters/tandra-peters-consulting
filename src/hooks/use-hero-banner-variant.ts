...
export const useHeroBannerVariant = (heroStyle?: HeroProps["heroStyle"]) => {
  ...
  if (forcedVariant) {
    return { isResolved: true, variant: forcedVariant };
  }

  if (liveVariant) {
    return { isResolved: true, variant: liveVariant };
  }
  ...
  if (cached) {
    return { isResolved: flagsReady, variant: cached };
  }

  return { isResolved: flagsReady, variant: undefined };
};

...
const faqAnswer = (...paragraphs: string[]): PortableTextBlock[] =>
  paragraphs.map((text, index) => ({
    ...
    markDefs: [],
    children: [{ _type: "span" as const, _key: `wf-faq-s-${index}`, text, marks: [] }],
  }));
...
const faqQuote = (text: string): PortableTextBlock[] => [
  {
    ...
    markDefs: [],
    children: [{ _type: "span" as const, _key: "wf-faq-quote-span", text, marks: [] }],
  },
];
...
export const INSURANCE_FAQ_ITEMS: FaqItem[] = [
  ...
  {
    _key: "wf-faq-2",
    question: "Does providing a contractor with my loss sheet harm me in any way?",
    answer: faqAnswer(
  ...
  {
    _key: "wf-faq-6",
    question: "When does my insurance company send me the Recoverable Depreciation payment?",
    answer: faqAnswer(

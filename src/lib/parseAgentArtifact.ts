...
export const parseAgentArtifactText = (text: string): AgentArtifactSegment[] => {
  ...
  while ((match = ARTIFACT_BLOCK_RE.exec(trimmed)) !== null) {
    const preamble = normalizeSegmentContent(trimmed.slice(lastIndex, match.index));
    if (preamble) {
      segments.push({ type: "markdown", content: preamble });
    }

    const title = (match[1] ?? match[2] ?? match[3] ?? "Suggested reply").trim();

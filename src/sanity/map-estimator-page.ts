...
export type EstimatorPageDoc = Record<string, unknown> | null | undefined;
...
const mapOption = (raw: Record<string, unknown>): EstimatorOption | null => {
  ...
  if (typeof raw._key === "string") {
    option._key = raw._key;
  }
...
const mapQuestion = (
  ...
  const question: EstimatorQuestion = { options, prompt };
  if (typeof raw._key === "string") {
    question._key = raw._key;
  }
  const helpText = cleanString(raw.helpText);
  if (helpText) {
    question.helpText = helpText;
  }
  if (raw.drivesSquareFootage === true) {
    question.drivesSquareFootage = true;
  }
  if (raw.multipliesSquareFootage === true) {
    question.multipliesSquareFootage = true;
  }
  return question;
};

export const mapEstimatorPageContent = (
  data: EstimatorPageDoc
): EstimatorPageContent => {
  if (!data) {
    return {};
  }

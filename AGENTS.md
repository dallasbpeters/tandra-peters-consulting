...
- Do not claim a change is done without either (a) showing the exact code path that makes it work and explaining why, or (b) providing a concrete manual verification checklist the user can run. Never say "this should work" without evidence.
...
- Root package runs **TypeScript 7.0.2** (Go-native compiler, ~6.6× faster than 5.8: 9.2s → 1.4s wall time on `tsc --noEmit`). `pnpm exec tsc --noEmit` and `pnpm lint` both invoke TS 7. `typescript-eslint` and other API consumers resolve `typescript` to `@typescript/typescript6` (6.0.3) via the `"typescript": "npm:@typescript/typescript6"` alias — `tsc6` is its binary. No `ignoreDeprecations` is set; `tsconfig.json` explicitly opts in to `strict: true` and lists `types: ["vitest/globals", "node"]`. Some server-render `@remotion/{renderer,effects,licensing}` packages resolve to `4.0.477` from `~/node_modules` vs the project's `4.0.466` (`npx remotion versions`) — a pre-existing env condition; `remotion bundle` still succeeds.
...
- **RoofInspection component** (`Diagram.tsx`) is a **3D-only** (`@google/model-viewer`) render of `public/roof.glb`; the 2D mode was removed. Invariants:
  1. Load model-viewer lazily via `import('@google/model-viewer')` in `Diagram.tsx` (not a separate `/public/model-viewer.min.js` script) so Vite shares one `three` instance with `shaders` — a vendored script bundle triggers `THREE.WARNING: Multiple instances of Three.js being imported`; `vite.config.ts` sets `resolve.dedupe: ['three']`.
  2. Chapters/labels/coords come from Sanity `homePage.roofInspection.hotspots` via `mapSanityHome` → `Home.tsx` (not `Rail.tsx` directly).
  3. Sanity `roofInspectionHotspot`: **`label`** = left rail nav; **`calloutTitle`** = hotspot popup headline — separate fields.
  4. Hotspots must be **slotted children** of `<model-viewer>` with `slot="hotspot-N"`, `data-position`, and `data-normal` (metres, model-specific — re-exporting `roof.glb` can shift world origin); filter chapters without `position3d` upstream before rendering `<RoofInspection.Hotspot>` to avoid React hooks violations.
  5. `model-viewer` caches slot positions — remount hotspots when coords change (Sanity `_key` + coord React `key`) and sync `data-position`/`data-normal` on prop change; do not backfill `position3d` from `CHAPTERS`/`data.ts` when Sanity hotspots exist. Publish Home page after coord edits (or use draft preview with read token).
  6. Hotspot callouts render via **React Portal** into `document.body` (not inside the slotted div), positioned from `getBoundingClientRect`; set `contain="none"` inline style on the `<model-viewer>` element so portaled cards escape all shadow DOM clipping.
  7. Camera attributes (`camera-orbit`, `camera-target`, `field-of-view`) must be set via `useState` initializers — never derive them from render-time state/context or React will silently overwrite imperative camera changes on every re-render.
  8. The `chapters` array in `Home.tsx` is rebuilt inline each render; use a `chaptersRef` in `Diagram.tsx` and keep it out of `useEffect` dependency arrays.
  9. `activeChapterId` (callout open/close — set by hover AND click) and `focusChapterId` (camera movement — set by rail clicks ONLY) are intentionally separate; hotspot hover must NEVER set `focusChapterId`.
  10. Context is split into two providers: `RoofInspectionContext` (chapters, activeChapterId — consumed by Hotspot and Rail) and `CameraContext` (focusChapterId, views, activeViewId — consumed by Diagram and Toolbar); this prevents Diagram from re-rendering on hover.
  11. Hotspot close uses a **module-level shared timer** (not per-instance `useRef`) so moving the cursor from one hotspot to another correctly cancels the pending close.

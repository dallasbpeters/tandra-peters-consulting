/**
 * Vercel serverless: POST /api/estimate → emails a roof estimate to the visitor
 * AND a lead notification to Tandra (Resend), then saves the visitor to Sanity.
 *
 * Env (Vercel → Project → Settings → Environment Variables):
 *   RESEND_API_KEY          — Resend API key.
 *   EMAIL_FROM              — Verified sender, e.g. "Tandra Peters <tandra@tandra.me>".
 *   CONTACT_NOTIFICATION_TO — Optional. Comma-separated lead recipients.
 ...
 *   SANITY_WRITE_TOKEN      — Optional. When set, the visitor is saved to the
 *     (or SANITY_API_WRITE_TOKEN)  Sanity `emailContact` list. Non-fatal if missing.
 *   ALLOWED_ORIGINS         — Optional. Comma-separated exact Origin values.
 */
import type { VercelRequest, VercelResponse } from "@vercel/node";

import { processEstimateSubmission } from "./email/estimateSubmission.js";
...
const estimateHandler = async (req: VercelRequest, res: VercelResponse): Promise<void> => {
  const origin = req.headers.origin as string | undefined;

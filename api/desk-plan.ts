/**
 * Vercel serverless: POST /api/desk-plan.
 *
 * LLM planner for Tandra's Desk board. Reads the current board, then uses Groq
 * (marketing-psychology mental models for tasks, content-strategy for content)
 * to generate the next batch of items and persists them as generated drafts.
 * Google-auth gated to the allowlist like /api/desk-board.
 *
 *   POST { mode: "tasks" | "content" } → { ok, items: [...] }
 */
...
const deskPlanHandler = async (
  ...
  const result = await generateBoardPlan({
    ...
    mode: normalizeMode(parseBody(req).mode),
...
export default async function handler(
  ...
  try {
    await deskPlanHandler(req, res);
  } catch (error) {
    console.error("[api/desk-plan]", error);
    if (!res.headersSent) {
      const origin = req.headers.origin as string | undefined;
      applyCors(res, origin);
      res.status(500).json({

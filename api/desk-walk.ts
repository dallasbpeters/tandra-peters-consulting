/**
 * Vercel serverless: POST /api/desk-walk.
 *
 * Per-home visit state (status / notes / follow-up) for the Desk "Walk area"
 * view. One `deskWalkVisit` Sanity doc per home, written as drafts (auth-gated,
 * no homeowner names). Mirrors /api/desk-targets.
 *
 *   POST { action: "list",   targetId }                 → { ok, visits[] }
 *   POST { action: "upsert", targetId, homeKey, status, notes?, followUpDate? }
 *                                                        → { ok, visit }
 */
import type { VercelRequest, VercelResponse } from "@vercel/node";

import { listWalkVisits, upsertWalkVisit } from "./lib/desk-walk.js";
...
export default async function handler(
  ...
  try {
    await deskWalkHandler(req, res);
  } catch (error) {
    console.error("[api/desk-walk]", error);
    if (!res.headersSent) {
      const origin = req.headers.origin as string | undefined;
      applyCors(res, origin);
      res.status(500).json({

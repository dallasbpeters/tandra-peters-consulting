/**
 * Vercel serverless: GET/POST/DELETE /api/desk-targets.
 *
 * Saved door-hanger canvassing targets for the Desk canvassing planner. These
 * are written as Sanity drafts (aggregate neighborhood signals only, no PII).
 * Google-auth gated to the allowlist, like /api/desk-capture.
 *
 *   GET                 → { ok, targets: [...] }
 *   POST  { name, ... } → { ok, target }        (create or update by id)
 *   DELETE ?id=...      → { ok }
 */
...
const deskTargetsHandler = async (
  req: VercelRequest,
  res: VercelResponse
): Promise<void> => {
  ...
  const result = await saveCanvassTarget(parseBody(req), {
    createdBy,
    sanityWriteToken,
  });
  res.status(result.status).json(result.body);
...
export default async function handler(
  ...
  try {
    await deskTargetsHandler(req, res);
  } catch (error) {
    console.error("[api/desk-targets]", error);
    if (!res.headersSent) {
      const origin = req.headers.origin as string | undefined;
      applyCors(res, origin);
      res.status(500).json({

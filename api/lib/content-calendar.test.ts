...
const createMock =
  vi.fn<(doc: Record<string, unknown>) => Promise<Record<string, unknown>>>();
const fetchMock =
  vi.fn<(...args: unknown[]) => Promise<Record<string, unknown>[]>>();
const deleteMock = vi.fn<(id: string) => Promise<unknown>>();
const commitMock = vi.fn<() => Promise<Record<string, unknown>>>();
const setMock = vi.fn<
  ...
  describe("createCalendarEntry", () => {
    ...
    it("creates a draft document and echoes the record", async () => {
      ...
      expect(doc).toMatchObject({
        _type: "deskBoardItem",
        createdBy: "dallas@example.com",
        detail: "Walk through what an adjuster looks for",
        kind: "content",
